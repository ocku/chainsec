...
Cache contents may be corrupt or contain malicious filesystem objects before ChainSec starts. Place the configured cache and its sibling `<cache>.locks` coordination directory in a parent controlled by the scanning principal; do not allow other principals to write or replace them. ChainSec validates that the opened cache root is owned by the effective user and is not group- or world-writable, and that the lock directory is owned by the effective user with mode `0700`. These checks and the locks are not an access-control boundary: cache locks coordinate only cooperating ChainSec processes and do not stop an arbitrary process that can ignore the protocol or mutate the containing filesystem.

ChainSec compiles only for Unix targets because fetching, extraction, cache, and workspace operations require descriptor-relative filesystem primitives. Path components below an opened root are resolved relative to its directory descriptor with no-follow semantics. Once a fetcher opens its cache directory, later rename or replacement of the configured cache pathname cannot redirect those operations; the fetcher does not recheck pathname identity or fail closed, and continues against its pinned open directory instead.
...
Cache keys are SHA-256 hashes of ecosystem, canonical name, resolved version/revision, integrity, and any source URL pinned before acquisition; Deno HTTP graph keys also include an immutable parsed snapshot digest of the applicable lockfile. Retained entries are copied into temporary sibling directories and renamed only after a completion manifest is written.

Each `SourceFetcher` opens and validates its cache root once, acquires a shared lifecycle lock at sibling `<cache>.locks/lifecycle.lock`, and keeps the open directory descriptor and lock for its lifetime. Cache acquisition, staging, reads, and publication remain descriptor-relative beneath that pinned root. If the configured pathname is later renamed or replaced, the existing fetcher neither switches to the replacement nor fails because the pathname identity changed; it continues against the original opened directory. This is a confinement property, not detection or prevention of pathname replacement.

Per-identity lock files in `<cache>.locks` coordinate cooperating cache readers and publishers. A valid existing winner is retained and the losing publisher discards only its own staging directory; an invalid or incomplete entry is atomically renamed aside and replaced while the exclusive per-identity lock prevents cooperating cache readers from traversing it. Cache reads reject symlinked cache components, entries, and markers, as well as malformed completion metadata and package identity mismatches. Restoration type is derived from trusted dependency semantics rather than completion metadata.

`chainsec cache purge` opens and validates the regular directory at the configured cache path for that purge and acquires `<cache>.locks/lifecycle.lock` exclusively. After cooperating active fetchers release their shared lifecycle locks, purge removes every child of its opened cache directory and stale per-identity lock files while retaining the cache directory, sibling lock directory, lifecycle lock, and unrelated lock-directory files. If the configured pathname was replaced before the later purge opened it, that purge targets the directory then at the configured path, not the detached directory pinned by an earlier fetcher; the detached directory can therefore remain after purge. A symlink or non-directory at the purge path is rejected. Lifecycle and per-identity locks coordinate only cooperating ChainSec processes and provide no protection from an arbitrary process that ignores them.
...
Each report schema version is an exact contract, not a compatibility range. Consumers must check and explicitly support the full `schema_version`; sharing a major version does not imply compatibility. Before the `chainsec` tool reaches `1.0.0`, minor report schema revisions may contain documented breaking changes, including required-field additions, removals, renames, or changes in meaning or type. Every schema revision must include explicit changelog and migration notes, and consumers must migrate before accepting that version.
