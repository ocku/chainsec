# Configuration and CLI/CI reference
...
- `--fail-on-parse-error`: record Tree-sitter-recovered syntax errors as fatal operational issues. ChainSec still analyzes the recovered syntax tree, retains any findings, and continues scanning other files and packages. Without this option, recovered parse errors are logged only at debug level.
- `--max-package-depth`, `--max-packages`, `--max-network-requests`, `--max-acquisition-seconds`, `--max-archive-size`, `--max-extracted-size`, `--max-extracted-files`, `--max-source-file-size`, `--max-scan-seconds`: resource controls (defaults: `3`, `4096`, `1000`, `300`, `100MiB`, `500MiB`, `50000`, `20MiB`, `300`). `--max-network-requests` and `--max-acquisition-seconds` apply separately to each package acquisition and include redirects and JSR's manifest-driven per-file downloads; concurrent packages do not share counters or deadlines. `--max-packages` caps each ordinary traversal and, for a remote version diff, both selected roots and aggregate unique root/dependency work across the whole batch. Size values accept bytes and human-readable `K`, `M`, `G`, and `T` forms, including `100m`, `100M`, `100MB`, and `100MiB`; all are binary (1024-based) units, and fractional values such as `1.5G` are accepted.
- `--threads <THREADS>`: maximum concurrency used for package downloads, package analysis, and source-file analysis (default `16`; minimum `1`). It can also be set with the `threads` configuration key; the command-line value takes precedence. This performance control does not change report contents or finding policy.
...
- `chainsec cache purge`: open the regular directory at the configured cache path, coordinate exclusively with cooperating fetchers through sibling `<cache>.locks/lifecycle.lock`, and delete every child of the opened cache directory while retaining that directory and the lifecycle lock. If the configured pathname was renamed or replaced, an existing fetcher remains pinned to its previously opened directory; a later purge targets the directory it opens at the configured path, not that detached original. The locks do not constrain arbitrary processes that ignore them. Combine with `--cache <dir>` to purge a specific cache.
...
max_source_file_size = 20971520  # 20 MiB
max_source_files = 100000
max_findings = 100000
max_scan_seconds = 300
# Continue analysis in either mode; true reports recovered parse errors as fatal.
...
All scalar and list scan options except the target `path`, remote package selector, and one-off `--verbose` flag have corresponding configuration keys: `max_package_depth`, `max_packages`, `max_network_requests`, `max_redirect_hops`, `request_timeout_seconds`, `max_acquisition_seconds`, `max_archive_size`, `max_extracted_size`, `max_extracted_files`, `max_file_depth`, `max_manifest_file_size`, `max_source_file_size`, `max_source_files`, `max_findings`, `max_scan_seconds`, `fail_on_parse_error`, `threads`, `cache`, `allow_unlocked`, `trust_local_input`, `allow_insecure_http`, `online`, `allowed_hosts`, `rule_packs`, `no_default_rules`, `ignored_rules`, `ignored_paths`, `format`, `fail_on`, and `output`. The `suppressions` table is configuration-only. Size limits in TOML are integer byte counts; the CLI alone accepts human-readable units such as `100MiB`. For backward compatibility, the 0.4 keys `max_depth`, `max_archive_bytes`, `max_extracted_bytes`, and `max_source_file_bytes` remain accepted as aliases for `max_package_depth`, `max_archive_size`, `max_extracted_size`, and `max_source_file_size`, respectively. Other unknown keys, malformed rule selectors, empty suppression reasons, and malformed ignored package selectors fail the scan before analysis.
